...
router.post("/", async (request, response) => {
  ...
  const verificationLink = `https://androidnow-zl4cootq3q-uc.a.run.app/verification/primary-mail/verify-link?emailAddress=${emailAddress}&id=${userAccountId}&publicKey=${verificationData.key}`;
  ...
  const mailResult = await mailjet
    ...
    .request({
      Messages: [
        {
          From: {
            Email: "admin_developer@efficientbytes.app",
            Name: "App Ingene",
          },
          To: [
            {
